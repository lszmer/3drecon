...
class ReconstructionDataIO:
    ...
    def load_colorless_mesh(self) -> Optional[o3d.t.geometry.TriangleMesh]:
        colorless_mesh_path = self.reconstruction_path_config.get_colorless_mesh_path()

        if not colorless_mesh_path.exists():
            return None
        
        return o3d.t.io.read_triangle_mesh(str(colorless_mesh_path))
    

    def save_colorless_mesh_legacy(self, mesh: o3d.geometry.TriangleMesh):
        colorless_mesh_path = self.reconstruction_path_config.get_colorless_mesh_path()
        colorless_mesh_path.parent.mkdir(parents=True, exist_ok=True)

        o3d.io.write_triangle_mesh(str(colorless_mesh_path), mesh)
    

    def save_colorless_mesh(self, mesh: o3d.t.geometry.TriangleMesh):
        colorless_mesh_path = self.reconstruction_path_config.get_colorless_mesh_path()
        colorless_mesh_path.parent.mkdir(parents=True, exist_ok=True)

        o3d.t.io.write_triangle_mesh(str(colorless_mesh_path), mesh)
    ...
    def load_colorless_pcd(self, device: o3d.core.Device = o3d.core.Device("CPU:0"), print_progress: bool = True) -> Optional[o3d.t.geometry.PointCloud]:
        ...
        # Temporary workaround: load without options, and filter manually if needed
        return o3d.t.io.read_point_cloud(filename=str(colorless_pcd_path), print_progress=print_progress).to(device=device)
    

    def save_colorless_pcd(self, pcd: o3d.t.geometry.PointCloud):
        colorless_pcd_path = self.reconstruction_path_config.get_colorless_pcd_path()
